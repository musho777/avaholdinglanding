        ...
        entries.forEach((entry) => {
          ...
          if (!entry.isIntersecting && entry.boundingClientRect.top < 0) {
            ...
            if (topTextRef.current) {
              topTextRef.current.classList.remove("scroll-reveal", "scroll-revealed", "delay-1", "century-enter-from-left");
              topTextRef.current.classList.add("century-exit-left");
            }
            if (bottomTextRef.current) {
              bottomTextRef.current.classList.remove("scroll-reveal", "scroll-revealed", "delay-2", "century-enter-from-right");
            ...
            if (hasExited) {
              ...
              if (topTextRef.current) {
                topTextRef.current.classList.remove("century-exit-left", "scroll-reveal", "delay-1");
                topTextRef.current.classList.add("century-enter-from-left");
              }
              if (bottomTextRef.current) {
                bottomTextRef.current.classList.remove("century-exit-right", "scroll-reveal", "delay-2");
              ...
              if (bottomTextRef.current) {
                bottomTextRef.current.classList.remove("century-exit-right", "century-enter-from-right");
    ...
    <section ref={sectionRef} className="century-section">
      ...
      <div className="century-scroll-indicator">
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="72.5" viewBox="0 0 10 72.5">
          <path d="M17.5,0l-.909.909,3.442,3.442H-50v1.3H20.032L16.591,9.091,17.5,10l5-5Z" transform="translate(10 50) rotate(90)" fill="#fff"></path>
        </svg>
      </div>
    </section>
  );
